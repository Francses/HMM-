...
class CNSegmentation():
    ...
    def buildInitProb(self):
        ...
        for value in self.tags_n:         #value分别为'b', 'B', 'M', 'E', 'S', 'e'
            idx = self.states.index(value)
            self.pi.append(self.transitionProb[idx_s, idx] * self.tags_n[value] / sum_tag)#状态转移概率乘发射概率
    ...
    def cutSentence(self, sentence,sep='。'):  # sentence 为分词后的数组形式
        ...
        print(windows) #一个句子加上其后面的一个标点为一个windows
